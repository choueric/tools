...
int DDC_Probe()
{
    ...
    if (!probe_ddc_edid(fd, buf, EDID_HEADER_END + 1))
        return -2;

	if (edid_check_header(buf)) {
    	debug_print("probed but dont't match EDID version 1 header\n");
        return -3;
    }
...
{
	struct edid_info *pinfo = &EdidInfo;
	unsigned char *buf = EdidBuf;
	int fd = Ddcfd;
...
#ifdef DEBUG
	dump_buf(buf, EDID_LEN);
#endif

	if (!parse_ddc_edid(buf, pinfo)) {
		printf("parse_ddc_edid() failed\n");
		return NULL;
	}
