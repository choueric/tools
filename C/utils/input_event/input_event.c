...
static void parse_event(char *str, unsigned short type, unsigned short code)
{
	sprintf(str, "%s.%s", events[type] ? events[type] : "?",
...
int main()
{
	...
	while ( (n = read(fd, &ev, sizeof(ev))) > 0) {
		parse_event(str, ev.type, ev.code);
		printf("[%d] [%ld.%ld]: %s, value = %d\n", count++,
				ev.time.tv_sec, ev.time.tv_usec, str, ev.value);
	}
