...
#define OPCODE_REMS         0x90   // read electronic manufacturer and device ID
...
static int do_rems(int fd)
{
	uint8_t buf[4] = {OPCODE_REMS, 0x00, 0x00, 0x01};
	uint8_t data[2] = {0};

	spi_write_then_read(fd, buf, 4, data, 2);
	printf("%02x%02x\n", data[0], data[1]);
	return 0;
}
